...
export function royaleApiCardUrl(card, variant = "base") {
  ...
  if (!slug) return null;
  // "evolutionLevel" (1/2/3) è solo il potenziamento dell'evoluzione, non un
  // disegno diverso: su RoyaleAPI esiste sempre e solo "-ev1" per ogni carta,
  // a qualunque livello sia stata potenziata. Usare il livello reale nel
  // nome del file (es. "-ev2") fa 403 e fa ripiegare sull'icona ufficiale
  // Supercell, più piatta — da qui l'incoerenza fra carte diverse.
  const suffix = variant === "evo" ? "-ev1" : variant === "hero" ? "-hero" : "";
...
// insegna di lega per la Classificata. Questa cartella di RoyaleAPI è
// rimasta alla numerazione PRE-aggiornamento (quando esistevano ancora le
// leghe Sfidante I-III, rimosse da Supercell — la Classificata parte ora
// direttamente da Maestro I): league4/5/6/7/8/9/10.png sono tuttora le
// icone giuste per Maestro I, II, III, Campione, Campione Grandioso,
// Campione Royale, Campione Definitivo, semplicemente sfasate di 3
// posizioni rispetto al leagueNumber 1-7 che l'API restituisce oggi.
// Verificato sia confrontando le descrizioni ufficiali delle insegne
// (Maestro II = martello, Maestro III = pozione, Campione = elmo, Campione
// Definitivo = diamante) sia con lo stato in gioco reale di un account
// (vedi lib/leagues.js).
export function leagueIconUrl(leagueNumber) {
  return leagueNumber ? `${UI_BASE}/leagues/league${leagueNumber + 3}.png` : null;
}
